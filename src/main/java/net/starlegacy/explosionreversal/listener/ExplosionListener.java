...
public record ExplosionListener(ExplosionReversalPlugin plugin) implements Listener {

    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    void onEntityExplode(EntityExplodeEvent event) throws Exception {
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    void onBlockExplode(BlockExplodeEvent event) throws Exception {
    ...
    private void processExplosion(World world, Location explosionLocation, List<Block> list) throws Exception {
    ...
    private void processBlock(List<ExplodedBlockData> explodedBlockDataList, double eX, double eY, double eZ,
                              Iterator<Block> iterator) throws Exception {
        ...
        @Nullable byte[] tileEntity = NMSWrapper.completeGetTileEntity(block);
    ...
    private void processTileEntity(List<ExplodedBlockData> explodedBlockDataList, Block block, long explodedTime) throws Exception {
    ...
    private void processDoubleChest(List<ExplodedBlockData> explodedBlockDataList, boolean isRight,
                                    DoubleChest doubleChest, long explodedTime) throws Exception {
        ...
        byte[] otherTile = NMSWrapper.completeGetTileEntity(other);
